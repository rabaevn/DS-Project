...
```{r}
# Columns to replace NA with mode
cols_to_replace <- c("does_the_content_present_advantages_and_disad", "are_there_local_examples_in_the_content_e_g_r", "access")
...
cleaned_data <- cleaned_data %>%
  mutate(across(all_of(cols_to_replace), ~ ifelse(is.na(.), get_mode(.), .)))
...
write.table(cleaned_data, file = "cleaned_data2.tsv", sep = "\t", row.names = FALSE, quote = FALSE)
```
